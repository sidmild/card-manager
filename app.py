...
# --- [2] 데이터 불러오기 및 필터링 ---
card_list = [c for c in card_sheet.col_values(2)[1:] if c]
user_list = [u for u in user_sheet.col_values(1)[1:] if u]

# 💡 구글 시트에서 품의 내용(A열)과 품의자(B열) 함께 불러오기
purpose_records = purpose_sheet.get_all_values()
purpose_list = []
if len(purpose_records) > 1:
    for row in purpose_records[1:]:
        if row[0]: # A열(품의 내용)이 비어있지 않은 경우
            # B열(품의자)에 이름이 적혀있으면 가져오고, 안 적혀있으면 빈칸 처리
            drafter = row[1] if len(row) > 1 and row[1] else "지정안됨"
            # "[품의내용] (품의자: OOO)" 형태로 글자 만들기
            purpose_list.append(f"{row[0]} (품의자: {drafter})")
    ...
    if not available_cards:
        ...
        if st.button("수령 완료", type="primary"):
            if purpose_content:
                ...
                # 선택한 글자 전체가 장부의 품의내용(E열)에 저장됩니다.
